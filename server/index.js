...
io.on('connection', (socket) => {
    ...
    socket.on('message', async ({
        ...
        try {
            ...
            if (_id && isInRoom(_id, updateRooms) && socket.isAuthenticated) {
                //create this message
                let count = await messagesModel.countDocuments({room:_id});
                console.log(count);
    ...
    //
    socket.on('getMessage', async ({_id,from,amount}, fn) => {
        try {
            ...
            if (_id && isInRoom(_id, updateRooms)) {
                ...
                let messages = await messagesModel.getMessages(_id, number, '-createdAt',from);
